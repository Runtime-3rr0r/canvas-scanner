...
1. Get the latest commit SHA publicly:
   ```bash
   curl -s https://api.github.com/repos/Runtime-3rr0r/canvas-scanner/commits?per_page=1 | grep -o '"sha": "[a-f0-9]*' | head -1
   ```
...
3. If newer:
   ...
   - **Zip/folder installs**: tell the user to re-download the repo zip (https://github.com/Runtime-3rr0r/canvas-scanner/archive/refs/heads/main.zip) and swap folders, OR re-clone.
